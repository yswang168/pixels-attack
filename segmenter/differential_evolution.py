...
class DifferentialEvolutionSolver(object):
    ...
    def _minimize_pixels(self, ep=0.5, try_all=True):
        if try_all:
           parameters = np.reshape(np.array([self._scale_parameters(trial) for trial in self.population]),(1,len(self.population)*len(self._bounds)))
        ...
        while m < lenp:
            ...
            parameters = np.reshape(np.array([self._scale_parameters(trial) for trial in trials]),(1,(m+1)*len(self._bounds)))
            best_p = self.func(parameters, *self.args)
            if best_p <= ep: break
            m += 1
        print('The minimal number of successful attacking pixels is %d with probability %f'%(self._num_pixels(), best_p))
    ...
    def _calculate_population_energies(self):
        ...
        # if type(energies)=='torch.Tensor':
        #     self.population_energies = energies.detach().cpu().numpy()
        # else:
