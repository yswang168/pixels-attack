...
def perturb_MRI(xs, img): ## img is a torch.tensor
    ...
    imgs = torch.Tensor.repeat(img, tile) ### np.tile(img.cpu(), tile)

    for x, img in zip(xs, imgs):
        # Split x into an array of 6-tuples (perturbation pixels)        
        pixels = np.split(x, len(x) // 6)
        for pixel in pixels: 
            x_pos, y_pos, *rgb = pixel
            for j in range(len(rgb)):
                img[j][int(x_pos), int(y_pos)] = rgb[j] 
    return imgs

def perturb_image(xs, img, RGB=True, MRI=False):
    # If this function is passed just one perturbation vector,
    # pack it in a list to keep the computation the same
    if xs.ndim < 2:
        xs = np.array([xs])
    ...
    # xs = xs.astype(int)  # revised by yisong for float value in [-1, 1]
...
def evaluate_models(models, x_test, y_test):
    ...
    return network_stats, correct_imgs, predictions # the last paremeter here is added by Yisong
